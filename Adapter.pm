...
#codigo refatorado e arrumado
sub nextoid {
    my ($self, $table, $arg) = @_;
    die "Table not defined" unless defined $table;
    $arg //= '';
...
sub allow_external_parent {
    my ($self, $allow_external_parent) = @_;
    $allow_external_parent //= 0;
    if ($allow_external_parent) {
        $self->{allow_external_parent} = 1;
    }
...
sub recreate_cache {
    my ($self, $recreate_cache) = @_;
    $self->{recreate_cache} = $recreate_cache if defined $recreate_cache;
...
sub first_feature_id {
    my ($self, $first_feature_id) = @_;
    if (defined $first_feature_id) {
        $self->{first_feature_id} = $first_feature_id;
    }
...
#Refatoração
sub nouniquecache {
    my ($self, $nouniquecache) = @_;
    if (defined $nouniquecache) {
        $self->{nouniquecache} = $nouniquecache;
    }
...
sub noexon {
    my ($self, $noexon) = @_;
    $self->{'noexon'} = $noexon if defined $noexon;
    return $self->{'noexon'};
}
...
sub score_col {
    my ($self, $col) = @_;
    $self->{'score_col'} = $col if defined $col;
...
sub analysis_group {
    my ($self, $anal) = @_;
    $self->{'analysis_group'} = $anal if defined $anal;
...
sub global_analysis {
    my ($self, $analysis) = @_;
    $self->{'global_analysis'} = $analysis if defined $analysis;
...
#Refatoração
sub is_analysis {
    my ($self, $is_analysis) = @_;
    $self->{'is_analysis'} = $is_analysis if defined $is_analysis;
...
sub save_tmpfiles {
    my $self = shift;
    my $save_tmpfiles = shift if defined $_[0];
    $self->{'save_tmpfiles'} = $save_tmpfiles if defined $save_tmpfiles;
...
#Refatoração
sub random_tmp_dir {
    my ($self, $random_tmp_dir) = @_;
    $self->{'random_tmp_dir'} = $random_tmp_dir if defined $random_tmp_dir;
...
#Refatoração
sub no_target_syn {
    my ($self, $no_target_syn) = @_;
    $self->{'no_target_syn'} = $no_target_syn if defined $no_target_syn;
...
#Refatoração
sub private_schema {
    my ($self, $private_schema) = @_;
    $self->{'private_schema'} = $private_schema if defined $private_schema;
...
sub use_public_cv {
    my $self = shift;
    return $self->{'use_public_cv'} = shift if @_;
...
sub dbxref {
    my ($self, $dbxref) = @_;
    $self->{'dbxref'} = $dbxref if defined($dbxref);
    return $self->{'dbxref'};
}
...
#Refatoração
sub primary_dbxref {
    my ($self, $primary_dbxref) = @_;
    $self->{'primary_dbxref'} = $primary_dbxref if defined($primary_dbxref);
...
#Refatoração
sub cds_db_exists {
    my ($self, $cds_db_exists) = @_;
    $self->{cds_db_exists} = $cds_db_exists if defined $cds_db_exists;
